    ...
    <div>
      <div class="flex items-center gap-3 mb-5">
        <h1 class="text-[#989898] font-normal text-[12px] cursor-pointer">
          <nuxt-link to="/sub-carrier">SUB CARRIER</nuxt-link>
        </h1>
...
export default {
  ...
  computed: {
    ...mapGetters({
      getSingleSubCarrierData: "sub-carrier/getSingleSubCarrierData",
  ...
  methods: {
    ...mapActions({
      updateSubCarrier: "sub-carrier/updateSubCarrier",
      fetchSingleSubCarrier: "sub-carrier/fetchSingleSubCarrier",
    ...
    getRoleValue(item) {
      ...
      this.formData.roleByCarrier = item.value;
    },
    async editSubCarrier() {
      try {
        this.errors = await this.$validateSubCarrierForm({
          form: this.formData,
        ...
        const response = await this.updateSubCarrier(this.formData);
        this.$toast.open({
          message: response.msg,
        });
        this.$router.push("/sub-carrier");
      } catch (error) {
        console.log(error);
        this.$toast.open({
  ...
  async asyncData({ params, store, redirect }) {
    ...
    try {
      await store.dispatch("sub-carrier/fetchSingleSubCarrier", {
        accountId: id,
      });
    } catch (error) {
      return redirect("/sub-carrier");
    }
  },
  async mounted() {
  ...
  beforeMount() {
    this.formData = this.$lodash.cloneDeep(this.getSingleSubCarrierData);
    const selectedRole = this.allRoleData.find(
      (role) => role._id === this.getSingleSubCarrierData.roleByCarrier
    );
    if (selectedRole) {
